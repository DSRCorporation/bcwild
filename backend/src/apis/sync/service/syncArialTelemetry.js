...
const observationToDtoProperties = {
  ...
  mesoSlopeId: { property: "mesoSlopeId" },
  macroPositionId: { property: "macroPositionId" },
...
const syncArialTelemetry = async (data) =>
  sequelize.transaction(async (transaction) => {
    ...
    const animal = await Animal.findOne(
      { where: { stringId: animalId } },
      { transaction },
    );
    if (!animal) {
      throw Error(`Animal with ID ${animalId} does not exist in the database`);
    }
    // Populate main table
